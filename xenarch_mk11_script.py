"""
Technosignature Detection: Xenarch Mk11 - Integrated Advanced Pipeline
======================================================================

Combines:
- VAE architecture from Mk5
- High-resolution visualization from Mk6
- Advanced false positive reduction (Mk7)
- Multi-metric anomaly scoring (Mk7)
- Spatial consistency filtering (Mk7)
- Adaptive thresholding (Mk7)
- IMPROVED: Edge regularity for boulder discrimination (Mk11)
- IMPROVED: Cluster-size based anomaly boosting (Mk11)

Usage:
    python xenarch_mk11_script.py
"""
...
class MultiMetricAnomalyScorer:
    ...
    def compute_edge_regularity(self, image):
        """
        Detect geometric/artificial edge patterns.
        Improved to distinguish between:
        - Random boulder clusters (natural, irregular spacing)
        - Artificial structures (precise geometry, straight lines)
        """
        scores = []
        ...
        for i in range(image.shape[0]):
            ...
            # Measure 1: Alignment (geometric = high alignment)
            ...
            # Measure 2: Edge continuity (straight lines have continuous edges)
            # Natural features (boulders) have broken/disconnected edges
            ...
            # Count pixels in largest connected edge component
            max_component_size = max([np.sum(labeled_edges == j) for j in range(1, num_features + 1)])
            continuity_score = max_component_size / strong_edges.sum()
            
            # Measure 3: Edge straightness (Hough-like)
            # Straight lines will have edges clustered at specific angles
            edge_angles = np.arctan2(gy[strong_edges], gx[strong_edges])
            if len(edge_angles) > 0:
                # Histogram of angles
                angle_hist, _ = np.histogram(edge_angles, bins=18)  # 10° bins
                angle_concentration = np.max(angle_hist) / len(edge_angles)
            else:
                angle_concentration = 0.0
            
            # Combine metrics - all must be high for artificial structures
            # Boulders: high alignment but LOW continuity and LOW angle concentration
            # Lander: high alignment AND high continuity AND high angle concentration
            
            # Require agreement across metrics (multiplicative)
            regularity_score = (
                alignment_score * 
                (continuity_score ** 0.5) *  # Square root to reduce weight
    ...
    def compute_combined_score(self, image):
        ...
        # Weighted combination - adjusted to prioritize reconstruction over geometric regularity
        # This prevents false positives from boulder clusters while still catching the lander
        combined = (
            0.40 * mse_norm +           # INCREASED - reconstruction error is most reliable
            0.25 * density_norm +       # INCREASED - latent space position matters
            0.25 * gradient_norm +      # Maintained - gradient mismatch is important
            0.10 * regularity_norm      # DECREASED - reduce false positives from boulder patterns
        )
...
class AdvancedAnomalyDetector:
    ...
    def detect_anomalies(
        ...
        if use_adaptive_threshold:
            ...
            adaptive = AdaptiveThreshold(chip_metadata, results_df['anomaly_score'].values)
            temp_df = adaptive.apply_adaptive_threshold(base_percentile=base_percentile)
            
            # Transfer adaptive results to results_df
            results_df['is_anomaly_adaptive'] = temp_df['is_anomaly_adaptive'].values
...
class HighResVisualizer:
    ...
    def plot_detection_analysis(self, results_df):
        ...
        metrics = ['metric_mse_norm', 'metric_density_norm', 'metric_gradient_norm', 'metric_regularity_norm']
        ...
        top_anomalies = results_df.nlargest(10, 'confidence')
        ...
        if len(metric_cols) > 0 and len(top_anomalies) > 0:
            ...
            ax6.set_title('Top 10 Anomalies - Average Metric Scores')
        ...
        plt.suptitle('Detection Analysis Dashboard (Mk11)', fontsize=18, fontweight='bold')
        plt.savefig(self.output_dir / 'analysis_dashboard_mk11.png', dpi=300, bbox_inches='tight')
...
def main():
    logger.info("="*70)
    logger.info("XENARCH Mk11: IMPROVED ADVANCED DETECTION PIPELINE")
    logger.info("="*70)
    ...
    data_root = Path("data")
    results_dir = Path("results") / "mk11_advanced"
    ...
    # Save model
    model_path = data_root / "models" / "xenarch_mk11.pth"
    ...
    # Save results
    results_df.to_csv(results_dir / "xenarch_mk11_results.csv", index=False)
    logger.info(f"Results saved: {results_dir / 'xenarch_mk11_results.csv'}")
    ...
    viz.plot_reconstruction_examples(model, test_ds, n_samples=6, device=config['device'])
    viz.plot_top_anomalies_with_confidence(results_df, n_samples=12)
    viz.plot_detection_analysis(results_df)

    logger.info("\n" + "="*70)
    logger.info("XENARCH Mk11 PIPELINE COMPLETE!")
    logger.info("="*70)
