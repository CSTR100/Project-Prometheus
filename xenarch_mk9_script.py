...
- Advanced false positive reduction (NEW)
- Multi-metric anomaly scoring (NEW)
- Spatial consistency filtering (NEW)
- Adaptive thresholding (NEW)

This version should reliably detect the lunar lander while dramatically
...
class MultiMetricAnomalyScorer:
    ...
    def compute_edge_regularity(self, image):
        ...
        for i in range(image.shape[0]):
            ...
            # Measure alignment (geometric = high alignment)
            ...
            regularity_score = max(row_alignment, col_alignment)
    ...
    def compute_combined_score(self, image):
        ...
        # Weighted combination - regularity weight increased for geometric structures
        combined = (
            0.30 * mse_norm +
            0.20 * density_norm +
            0.25 * gradient_norm +
            0.25 * regularity_norm  # High weight for geometric patterns!
        )
...
class SpatialConsistencyFilter:
    ...
    def apply_spatial_filtering(self, threshold_percentile=92, min_cluster_size=2):
        ...
        # Boost clustered anomaly scores
        self.df.loc[self.df['is_anomaly_filtered'], 'anomaly_score'] *= 1.2
...
class HighResVisualizer:
    ...
    def plot_detection_analysis(self, results_df):
        ...
        plt.suptitle('Detection Analysis Dashboard (Mk7)', fontsize=18, fontweight='bold')
        plt.savefig(self.output_dir / 'analysis_dashboard_mk7.png', dpi=300, bbox_inches='tight')
...
def main():
    logger.info("="*70)
    logger.info("XENARCH Mk7: INTEGRATED ADVANCED DETECTION PIPELINE")
    logger.info("="*70)

    # Configuration
    config = {
        'chip_size': 256,
        'latent_dim': 56,  # Optimized for balance
        'batch_size': 4,   # Small for 256×256
        'num_epochs': 10,
        'learning_rate': 0.001,
        'base_percentile': 92,
    ...
    data_root = Path("data")
    results_dir = Path("results") / "mk7_advanced"
    ...
    # Save model
    model_path = data_root / "models" / "xenarch_mk7.pth"
    ...
    # Save results
    results_df.to_csv(results_dir / "xenarch_mk7_results.csv", index=False)
    logger.info(f"Results saved: {results_dir / 'xenarch_mk7_results.csv'}")
    ...
    logger.info("\n" + "="*70)
    logger.info("XENARCH Mk7 PIPELINE COMPLETE!")
    logger.info("="*70)
    logger.info(f"Results directory: {results_dir}")
    logger.info(f"Total anomalies: {results_df['is_anomaly_final'].sum()}")
    logger.info(f"High confidence (>0.8): {(results_df['confidence'] > 0.8).sum()}")
    logger.info("\nCheck results/mk7_advanced/ for outputs:")
    logger.info("  - xenarch_mk7_results.csv (detailed results)")
    logger.info("  - reconstructions_mk7.png (reconstruction quality)")
    logger.info("  - top_anomalies_mk7.png (top detections by confidence)")
    logger.info("  - analysis_dashboard_mk7.png (comprehensive analysis)")
